...
class ObjectStoreUtil:
    def __init__(
        ...
        if self.objStoreHost is None:
            self.objStoreHost = config.OBJ_STORE_HOST
        if self.objStoreUser is None:
            self.objStoreUser = config.OBJ_STORE_USER
        if self.objStoreSecret is None:
            self.objStoreSecret = config.OBJ_STORE_SECRET
        if self.objStoreBucket is None:
            self.objStoreBucket = config.OBJ_STORE_BUCKET
        # populate a temp folder variable.. if none is provided as an
        # arg or in a config variable then just use the current
        # directory
        if self.tmpfolder is None:
            if hasattr(config, "TMP_FOLDER"):
                self.tmpfolder = config.TMP_FOLDER
            else:
    ...
    def createBotoClient(
        ...
        uses the following config to build the connection:
        Treat this as a private method.  Any other methods that need a boto
        client will call this first.
        client id:      config.OBJ_STORE_USER
        client secret:  config.OBJ_STORE_SECRET
        s3 host:        config.OBJ_STORE_HOST
        """
